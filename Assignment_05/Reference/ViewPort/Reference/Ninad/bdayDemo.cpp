...
Title: PIXEL PROJECT: Fixed Function Pipeline- Viewport Demo

1. Create the basic window.
2. Turn on the fullscreen functionality.
3. Add further changes which need to be done with respect to OpenGL:
a. initialize()
b. display()
c. resize()
...
//For Fragment Shader
GLfloat gAngleTri = 0.0f;
GLfloat colorChange_r = 1.0f;
GLfloat colorChange_g = 1.0f;
GLfloat colorChange_b = 1.0f;

bool gbAnimeDone = false;
...
void resize(int width, int height)
{
	...
	case 4://Right bottom Corner
		...
		glViewport((GLint)width / 2, 0, (GLsizei)width / 2, (GLsizei)height / 2);
	...
	case 6://Right half of the screen
		...
		glViewport((GLint)width / 2, 0, (GLsizei)width / 2, height);

		break;
	case 7://Top half of the screen
		fprintf(gpFile, "iKeyPressed=%d\n", iKeyPressed);

		glViewport(0, (GLint)height / 2, width, (GLsizei)height / 2);
	...
	case 9://Center
		fprintf(gpFile, "iKeyPressed=%d\n", iKeyPressed);
		glViewport((GLint)width / 4, 0, (GLsizei)width / 2, (GLsizei)height);
		break;
	...
	default:
			...
			if (gbAnimeDone == 1)
			{
				glViewport((GLint)width / 2, (GLint)height / 2, (GLsizei)width/2, (GLsizei)height/2);
			}
			else
			{
			...
			gluPerspective(45.0f, (GLfloat)width / (GLfloat)height, 0.1f, 100.0f);
			 ...
			 viewportHeight = (GLfloat)height;
			 viewportWidth = (GLfloat)width;
...
void ForFragmentShader(void)
{
	...
	glRotatef(gAngleTri, 0.0f, 1.0f, 0.0f);

	glBegin(GL_TRIANGLES);
	//NOTE : EACH FACE OF A PYRAMID (EXCEPT THE BASE/BOTTOM) IS A TRIANGLE

	//****FRONT FACE****
	glColor3f(1.0f, colorChange_r, colorChange_r);
	glVertex3f(0.0f, 1.0f, 0.0f);

	glColor3f(colorChange_g, 1.0f, colorChange_g);
	glVertex3f(-1.0f, -1.0f, 1.0f);

	glColor3f(colorChange_b, colorChange_b, 1.0f);
	glVertex3f(1.0f, -1.0f, 1.0f);
...
void animate(void)
{
	void animateViewPortFragmentShader(void);
	if (colorChange_r >= 0.0f)
		colorChange_r -= 0.003f;
	else if (colorChange_g >= 0.0f)
		colorChange_g -= 0.003f;
	else if (colorChange_b >= 0.0f)
		colorChange_b -= 0.003f;
	else
	{
		spin();
		animateViewPortFragmentShader();
	}
}
void animateViewPortFragmentShader(void)
{

	if (gbAnimeDone == false)
	{
		if (viewportHeight > (fHeight / 2.0f))
		{
			viewportHeight -= (fWidth / fHeight)*(fHeight / 1000.0f);
		}
		
		if (viewportWidth > fWidth / 2.0f)
		{
			viewportWidth -= (fWidth / fHeight)*(fWidth / 1000.0f);
		}
		if (viewport_x < fWidth / 2.0f)
		{
			viewport_x += (fWidth / fHeight)*(fWidth / 1000.0f);
		}
		if (viewport_y < fHeight / 2.0f)
		{
			viewport_y += (fWidth / fHeight)*(fHeight / 1000.0f);
		}
		else
		{
			gbAnimeDone = 1;
			fprintf(gpFile, "viewportwidth=%f\nviewportheight=%f\nheight=%f\nwidth=%fAnimebool=%d\n", viewportWidth, viewportHeight, fWidth, fHeight, gbAnimeDone);
		}
		glViewport((GLint)viewport_x, (GLint)viewport_y, (GLsizei)viewportWidth, (GLsizei)viewportHeight);
